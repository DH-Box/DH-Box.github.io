...
module Jekyll
  class SiteMenus < Liquid::Tag
    ...
    def renderMenu(context, menu, level)
      ...
      if (isFirstLvl)
        output += "<ul id=\"#{@menu_name}-menu\" class=\"nav navbar-pills menu level-#{level}\">\n"
      else
#        output += "<div class=\"sub-menu level-#{level}\"><ul class=\"menu sub-menu level-#{level} nav navbar-pills\">\n"
        output += ""
      end
